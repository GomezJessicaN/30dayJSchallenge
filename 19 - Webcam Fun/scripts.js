...
function getVideo() {
    navigator.mediaDevices
        ...
        .catch(err => {
            console.error(`OH NO! maldita sea`, err);
        });
}
...
function takePhoto() {
    ...
    link.href = data;
    link.setAttribute('download', 'IUPIIIII');
    link.innerHTML = `<img src="${data}" alt="MIRA UNA FOTO MIRA" />`;
...
function rgbSplit(pixels) {
    for (let i = 0; i < pixels.data.length; i += 4) {
        ...
        pixels.data[i - 150] = pixels.data[i + 2]; //Azul como el mar azul
    }
    return pixels;
}
